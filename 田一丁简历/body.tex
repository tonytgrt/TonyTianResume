...
\datedline{\textbf{ButterFlight - 蝴蝶飞行动画创作工具} $|$ \emph{Maya, C++, MEL}}{\dateRange{2026.02}{2026.05}}
\begin{itemize}
  \item 与团队共2人基于Chen et al.\ SIGGRAPH '22论文，开发Maya C++插件（\texttt{.mll}），实现蝴蝶飞行动画的自动物理模拟，使VFX艺术家无需手动设置关键帧或掌握空气动力学知识即可在场景中批量生成蝴蝶动画
  \item 实现基于论文的三模块物理管线：参数化5自由度关节角建模（胸部俯仰、翅膀拍打/羽化/扫掠，腹部以$-180^\circ$相位偏移驱动骨骼层级）；curl-noise涡流力场模拟真实蝴蝶飞行轨迹固有的混沌噪声
  \item 开发支持最多100个独立个体（含个体间排斥力）的群体模拟、NURBS路径跟随模式、随时间变化的风力交互，将结果烘焙为标准Maya \texttt{animCurve}关键帧并支持导出至Alembic或FBX
  \item 项目仓库：\url{https://github.com/tonytgrt/ButterFlight}
\end{itemize}
...
\begin{itemize}[parsep=0.5ex]
  \item 编程/API: C/C++, CUDA, OpenGL, GLSL, Vulkan, WebGPU, WGSL, Metal, MSL, MEL, Python
  \item 图形/渲染: NSight性能分析, 路径追踪，全局光照，渲染管线，延迟渲染, 物理模拟, 动画系统, PBR
  \item 软件/工具: Unreal (C++), Unity, Maya (MEL), NSight, Visual Studio, Houdini, Qt, Git, CMake
  \item 语言: 英语流利（托福110, ACT 36），日语入门（N5），粤语入门
\end{itemize}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
